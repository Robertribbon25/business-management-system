...
import { Plus, Edit, Trash2, X, RefreshCw, Search, Contact, MapPin } from 'lucide-react';
...
export default function Suppliers() {
  ...
  // Automatically and safely clear notifications
  useEffect(() => {
  ...
  return (
    <div className="space-y-6 text-slate-100">
      ...
      <div className="bg-white/5 backdrop-blur-md border border-white/10 p-4 rounded-xl shadow-lg">
        <div className="relative w-full max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-white/10 rounded-lg text-xs leading-none bg-white/5 text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:bg-white/10 transition font-medium"
          />
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse text-xs select-none">
              ...
              <tbody className="divide-y divide-white/5 font-semibold text-slate-300">
                ...
                {filteredSuppliers.length === 0 && (
                  <tr>
                    <td colSpan="6" className="p-8 text-center text-slate-500 italic">No supplier records match search queries.</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {/* Input Form Modal Overlay */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/65 backdrop-blur-sm p-4 text-slate-200">
          <div className="bg-slate-900 border border-white/15 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-6">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Supplier Name</label>
                <input
                  type="text"
                  name="name"
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Contact Person Name</label>
                  <input
                    type="text"
                    name="contactName"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Corporate Email</label>
                  <input
                    type="email"
                    name="email"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Phone</label>
                  <input
                    type="text"
                    name="phone"
                    placeholder="+250..."
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Warehouse Address</label>
                  <input
                    type="text"
                    name="address"
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Special Procurement Notes</label>
                <textarea
                  name="notes"
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-bold rounded-lg text-xs transition cursor-pointer"
                >
                  Save Supplier
                </button>
              </div>
